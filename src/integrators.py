...
class Integrator():
    ...
    def sim_paths_layered(self, S_paths, option, n_steps, theta = None):
        ...
        for step_i in range(n_steps):
            #print(n_steps, S_paths.level, dt, is_coarse_step_list[step_i], "&&&&&&&&&&&&&&&&&&&&&&")
...
if False:
    ...
    def sim_paths_layered1(self, S_paths, option, n_steps, theta=None):
        is_coarse_step_list = [((step_i % self.M == self.M - 1) and step_i > 0) for step_i in range(n_steps)]
        expiry = option.expiry
        ...
        if theta == None:
            ...
            if theta.shape[0] < n_steps:
                theta = tc.tensor([list(theta[0])] * n_steps, device=device0)
        if is_layered_sim:
            dW_t_sub = tc.full(dim, 0.0, device=device0)
            dWv_t_sub = tc.full(dim, 0.0, device=device0)


        if self.ImportanceSampling_training:
            S_paths.dW = tc.empty([n_steps, n_sims, n_assets], device=device0)
            # S_paths.dW  = [None] * n_steps
        #

        for step_i in range(n_steps):
            #print(dim, "here dim")
            #dW_t = sqrt_dt * self.sampler.normal(dim)
            #print(dW_t.shape)
            dW_t = sqrt_dt * tc.normal(0.0, 1.0, dim, device=device0)
            #dW_t = S_paths.rnd_normal_dW(step_i, dt)
            #dWv_t0 = sqrt_dt * tc.normal(0.0, 1.0, dim, device=device0)
            if self.ImportanceSampling:
                if self.ImportanceSampling_training:
                    S_paths.dW[step_i] = dW_t
                theta_t = theta[step_i]
                # print(S_paths.integ_theta_dW.device, theta.device)
                S_paths.integ_theta_dW = S_paths.integ_theta_dW + theta_t * dW_t
                S_paths.integ_thetasq_dt = S_paths.integ_thetasq_dt + (theta_t ** 2) * dt
                dW_t += theta_t * dt
                # S_paths.integ_theta_dW = S_paths.integ_theta_dW + theta[step_i] * dW_t
            ...
            dWv_t = rho * dW_t \
                    + sqrt_one_minus_rhosq * sqrt_dt * tc.normal(0.0, 1.0, dim, device=device0)
            #dWv_t = rho * dW_t \
            #        + sqrt_one_minus_rhosq * sqrt_dt * self.sampler.normal(dim)
            # S_paths.integ_voltheta_dWv = S_paths.integ_voltheta_dWv + voltheta_t * dWv_t0
            # S_paths.integ_volthetasq_dt = S_paths.integ_volthetasq_dt + (voltheta_t ** 2) * dt
    ...
    def sim_paths_layered1(self, S_paths, option, n_steps, theta=None):
        ...
        # if begineuler:
        #     if S_paths.level == 0:
        #         self.disc_step = self.disc_step2
        #         self.disc_step_sub = self.disc_step2
        #     if S_paths.level == 1:
        #         self.disc_step = self.disc_step1
        #         self.disc_step_sub = self.disc_step2
        #     if S_paths.level > 1:
        #         self.disc_step = self.disc_step1
        #         self.disc_step_sub = self.disc_step1
        ...
        dim = [n_sims, n_assets]
        n_slice = S_paths.n_slice
        # S_paths.n_steps = n_steps
        # S_paths.n_steps_sub = int(n_steps/self.M)
        # Importance_Sampling = True
        if theta == None:
            pass
            S_paths.is_importance_sampled = False
            ...
            S_paths.is_importance_sampled = True
            # S_paths.ImportanceSampling = True
        # if theta.shape[0] < n_steps:
        #     theta = tc.tensor([list(theta[0])] * n_steps, device=device0)
        # if is_layered_sim:
        #     dW_t_sub = tc.full(S_paths.dim_sub, 0.0, device=device0)
        ...
        dW_t = tc.empty(dim, device=device0)
        for step_i in range(n_steps):
            print(n_steps, S_paths.level, "&&&&&&&&&&&&&&&&&&&&&&")
            # dW_t = sqrt_dt * self.sampler.normal(dim)
            # dW_t.normal_().mul_(sqrt_dt)
            if step_i % self.M == 0 or S_paths.level == 0 or not S_paths.antithetic:
                dW_t.normal_().mul_(sqrt_dt)
                if S_paths.antithetic:
                    dW_t_store = dW_t.clone()
            elif S_paths.antithetic:
                dW_t_store = dW_t_store.roll(S_paths.n_sims_sub, 0)
                dW_t = dW_t_store
                # dW_t = dW_t.roll(S_paths.n_sims_sub, 0) - theta[0] * dt
            # dW_t = sqrt_dt * tc.normal(0.0, 1.0, dim, device=device0)

            if self.ImportanceSampling:
                if self.ImportanceSampling_training:
                    S_paths.dW[step_i] = dW_t[:S_paths.n_slice]
                    # S_paths.dW[step_i] = dW_t #[:S_paths.n_slice]

                # print(S_paths.integ_theta_dW.device, theta.device)
                S_paths.integ_theta_dW = S_paths.integ_theta_dW + theta[step_i] * dW_t  # [:S_paths.n_slice]
                # S_paths.integ_theta_dW = S_paths.integ_theta_dW + theta[step_i] * dW_t[:S_paths.n_slice]
                S_paths.integ_thetasq_dt = S_paths.integ_thetasq_dt + (theta[step_i] ** 2) * dt
                dW_t += theta * dt
                # dW_t += theta[step_i] * dt
                # S_paths.integ_theta_dW = S_paths.integ_theta_dW + theta[step_i] * dW_t
            ...
            # print("dW_t 0 @ step", step_i )
            # if S_paths.level > 0 and S_paths.antithetic:
            #     print("dW_t 0 @ step", step_i)
            #     print(dW_t[0], dW_t[S_paths.n_slice])
